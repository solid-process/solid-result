...
class BCDD::Result
  class Context::TransitionsRecursionTest < Minitest::Test
    module Fibonacci
      ...
      def require_valid_number(input)
        input.is_a?(Numeric) or return Failure(:invalid_arg, message: 'input must be numeric')

        input.negative? and return Failure(:invalid_arg, message: 'number cannot be negative')

        Success(:ok, number: input)
      end
    end
